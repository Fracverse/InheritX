...
use kyc_contract::{KycContractClient, KycError};
...
pub enum InheritanceError {
    ...
    KycNotApprovedCreate = 36,
    KycNotApprovedClaim = 37,
    KycContractNotSet = 38,
}
...
impl InheritanceContract {
    ...
    /// Set the KYC contract address. Only admin can call this.
    ...
    /// Get the KYC contract address.
    fn get_kyc_contract(env: &Env) -> Result<Address, InheritanceError> {
        env.storage()
            .instance()
            .get(&DataKey::KycContract)
            .ok_or(InheritanceError::KycContractNotSet)
    }

    /// Verify user has approved KYC status via cross-contract call.
    fn ensure_kyc_approved(
        env: &Env,
        user: &Address,
        for_claim: bool,
    ) -> Result<(), InheritanceError> {
        let kyc_contract = Self::get_kyc_contract(env)?;
        let kyc_client = KycContractClient::new(env, &kyc_contract);

        match kyc_client.try_require_approved(user) {
            Ok(()) => Ok(()),
            Err(_) => {
                if for_claim {
    ...
    /// - KycNotApprovedCreate: If owner's KYC status is not approved
    ...
    pub fn claim_inheritance_plan(
        ...
        // Get the caller address (beneficiary claiming)
