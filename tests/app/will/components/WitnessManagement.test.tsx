...
import WitnessManagement from '../../../../app/will/components/WitnessManagement';
...
describe('WitnessManagement', () => {
  ...
  it('verifies a signature on-chain', async () => {
    // initial fetch
    global.fetch = vi.fn()
      .mockResolvedValueOnce({ ok: true, json: async () => sampleWitnesses } as any)
      // verify POST
    ...
    // Click verify on Bob (already signed)
    const verifyButton = screen.getByText('Verify');
    fireEvent.click(verifyButton);

    await waitFor(() => expect(screen.getByText('✓ Verified on-chain')).toBeInTheDocument());
  });
});
